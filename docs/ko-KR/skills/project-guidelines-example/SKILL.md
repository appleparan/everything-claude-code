...
실제 프로덕션 애플리케이션 기반: [Zenith](https://zenith.chat) - AI 기반 고객 탐색 플랫폼.

---

## 사용 시점

프로젝트 특화 설계 작업 시 이 스킬을 참조하세요. 프로젝트 스킬에 포함된 내용:
- 아키텍처 개요
- 파일 구조
- 코드 패턴
- 테스트 요구 사항
...
- **AI**: Claude API (도구 호출 및 구조화된 출력 포함)
- **배포**: Google Cloud Run
...
1. **이모지 금지** - 코드, 주석, 문서에서
2. **불변성** - 객체나 배열을 절대 변경하지 않기
3. **TDD** - 구현 전 테스트 먼저 작성
4. **80% 커버리지** 최소
5. **여러 개의 작은 파일** - 200-400줄이 일반적, 최대 800줄
...
- `coding-standards.md` - 일반 코드 모범 사례
